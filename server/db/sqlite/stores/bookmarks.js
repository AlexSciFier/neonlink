export default class BookmarksStore {
  ...
  addItem(url, title, desc, icon, categoryId, tags) {
    const insertQuery = 
      `INSERT INTO bookmarks (url,title,desc,icon,search,categoryId) 
    ...
    const insertParams = { 
      ...
      categoryId }
    const bookmarkId = this.db.prepare(insertQuery).run(insertParams).lastInsertRowid;

    const tagsSelectQuery = `SELECT id FROM tags WHERE name IN (${new Array(tags?.length || 0).fill("?").join(",")})`;
  ...
  getAll(search = undefined, tag = undefined, category = undefined) {
    let selectQuery = 
      ...
      FROM bookmarks 
        ...
        ${category !== undefined ? "LEFT JOIN category ON category.id = bookmarks.categoryId" : ""}`;
  ...
  getPage(limit = 10, offset = 0, search = undefined, tag = undefined, category = undefined) {
    let countQuery = 
      `SELECT 
        COUNT(*) as count 
      FROM bookmarks
        ${tag !== undefined ? "LEFT JOIN bookmarksTags ON bookmarksTags.bookmarkId = bookmarks.id" : "" } 
        ${tag !== undefined ? "LEFT JOIN tags ON bookmarksTags.tagId = tags.id" : "" }
        ${category !== undefined ? "LEFT JOIN category ON category.id = bookmarks.categoryId" : "" }`;
  ...
  updateItem(id, url, title, desc, icon, categoryId, tags) {
    ...
    if (tags) {
      let tagsSelectQuery = `SELECT id FROM tags WHERE name IN (${new Array(tags.length).fill("?").join(",")})`;
    ...
    return this.db.prepare(updateQuery).run({ id, url, title, desc, icon, categoryId }).changes > 0;
  }
